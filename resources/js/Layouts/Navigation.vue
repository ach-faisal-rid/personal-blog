        ...
        <nav class="bg-white border-b border-gray-100 dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <div class="hidden sm:flex sm:items-center sm:ms-6">
                        <div class="relative ms-3">
                            <span v-if="!auth?.user" class="inline-flex space-x-4">
                                <!-- Login and Register Links for not authenticated user -->
                                <Link :href="route('login')" 
                                    class="inline-flex items-center px-4 py-2 text-sm font-medium leading-5 text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-blue-500 hover:text-white">
                                    Login
                                </Link>
                                <Link :href="route('register')" 
                                    class="inline-flex items-center px-4 py-2 text-sm font-medium leading-5 text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-green-500 hover:text-white">
                                    Register
                                </Link>
                            </span>
            ...
            <div :class="{ 'block': showingNavigationDropdown, 'hidden': !showingNavigationDropdown }" class="sm:hidden">
                <div class="pt-2 pb-3 space-y-1">
                    <ResponsiveNavLink v-if="auth?.user" :href="route('profile.show')" :active="route().current('profile.show')">
                        Profile
                    </ResponsiveNavLink>

                    <ResponsiveNavLink v-if="auth?.user" :href="route('dashboard')" :active="route().current('dashboard')">
                        Dashboard
                    </ResponsiveNavLink>
                    ...
                    <ResponsiveNavLink v-if="!auth?.user" :href="route('login')" class="text-gray-500 dark:text-gray-300">
                        Login
                    </ResponsiveNavLink>

                    <ResponsiveNavLink v-if="canRegister && !auth?.user" :href="route('register')" class="text-gray-500 dark:text-gray-300">
                        Register
                    </ResponsiveNavLink>
                </div>
            </div>
