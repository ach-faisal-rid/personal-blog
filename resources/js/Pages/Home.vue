...
<template>
    ...
    <div class="relative min-h-screen bg-gray-100 bg-center sm:flex sm:justify-center sm:items-center bg-dots-darker dark:bg-dots-lighter dark:bg-gray-900 selection:bg-red-500 selection:text-white">
        <div class="text-center">
            <h1 class="text-4xl font-bold text-gray-800 dark:text-white">Hello, Selamat Datang di Laravel</h1>
        </div>

        <div v-if="canLogin" class="z-10 p-6 sm:fixed sm:top-0 sm:end-0 text-end">
            ...
            <template v-else>
                ...
                <Link v-if="canRegister" :href="route('register')" class="font-semibold text-gray-600 ms-4 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">Register</Link>
            </template>
        </div>
